...
func TestSavePost(t *testing.T) {
	err := refreshUserAndPostTable()
	if err != nil {
		log.Fatalf("Error user and post refereshing table: %v\n", err)
	}
...
func TestGetPostByID(t *testing.T) {
	...
	if err != nil {
		t.Errorf("this is the error getting one post: %v\n", err)
		return
	}
...
func TestUpdateAPost(t *testing.T) {
	...
	postUpdate := models.Post{
		ID:       1,
		Title:    "modiUpdate",
		Content:  "modifier content for a post",
...
func TestDeleteAPost(t *testing.T) {
	...
	post, err := seedOneUserAndOnePost()
	if err != nil {
		log.Fatalln("Error seeding tables")
	}
